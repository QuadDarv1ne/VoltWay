...
class QueryProfiler:
    ...
    def profile(self, func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                execution_time = time.time() - start_time
                self._log_query(
                    func.__name__, execution_time, args, kwargs, error=str(e)
                )
                raise
    ...
    def _log_query(
        ...
        if error:
            ...
            logger.warning(
                f"Slow query in {func_name}: {execution_time:.4f}s - Error: {error}"
            )
    ...
    def get_stats(self) -> dict:
        ...
        execution_times = [
            q["execution_time"] for q in self.queries if "error" not in q
        ]
