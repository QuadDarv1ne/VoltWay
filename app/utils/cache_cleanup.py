...
class CacheCleanupManager:
    ...
    async def start_cleanup_scheduler(self):
        ...
        if not cache._redis_client or not cache._redis_client.is_connected():
    ...
    async def perform_cleanup_cycle(self):
        ...
        if total_cleaned > 0:
            logger.info(
                f"Cache cleanup cycle completed: {total_cleaned} entries cleaned"
            )
        else:
    ...
    async def _cleanup_by_size(self) -> int:
        ...
        try:
            ...
            logger.info(
                f"Cache size {current_size} exceeds limit {self.max_cache_size}, cleaning up {excess_count} entries"
            )
    ...
    async def _cleanup_by_memory(self) -> int:
        ...
        try:
            ...
            logger.info(
                f"Memory usage {used_memory} exceeds limit {self.max_memory_usage}, performing cleanup"
            )
            ...
            logger.info(
                f"Freed approximately {freed_memory} bytes by deleting {cleaned_count} entries"
            )
            return cleaned_count
        except Exception as e:
