...
def test_cache_set_success():
    ...
    with patch('redis.from_url') as mock_redis:
        ...
        with patch('app.utils.cache.pickle.dumps', return_value=b'serialized_value'):
            ...
            mock_redis_instance.setex.assert_called_once_with("test_key", 3600, b'serialized_value')
...
def test_clear_station_cache():
    ...
    with patch('redis.from_url') as mock_redis:
        mock_redis_instance = Mock()
        mock_redis_instance.ping.return_value = True
        mock_redis_instance.keys.return_value = ["stations:lat:55.7558:lon:37.6173", "stations:lat:56.0:lon:38.0"]
        ...
        mock_redis_instance.delete.assert_called_once_with("stations:lat:55.7558:lon:37.6173", "stations:lat:56.0:lon:38.0")
