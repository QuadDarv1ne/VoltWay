...
def test_cache_manager_initialization_success():
    ...
    with patch("app.utils.cache.redis_client.RedisClient") as mock_redis_client:
...
def test_cache_manager_initialization_failure():
    ...
    with patch("app.utils.cache.redis_client.RedisClient") as mock_redis_client:
        ...
        cache_manager = CacheManager()
        assert cache_manager._redis_client is None or cache_manager._use_memory_fallback
...
def test_cache_set_when_redis_unavailable():
    """Test that set returns True when writing to memory cache (Redis unavailable)"""
    cache_manager = CacheManager()
    cache_manager._redis_client = None

    result = cache_manager.set("test_key", "test_value")
    # Should return True because it writes to memory cache
...
def test_cache_get_found():
    ...
    with patch("app.utils.cache.manager.RedisClient") as MockRedisClient:
        mock_redis_instance = Mock()
        mock_redis_instance.is_connected.return_value = True
        ...
        with patch(
            "app.utils.cache.redis_client.pickle.loads",
...
def test_cache_set_success():
    ...
    with patch("app.utils.cache.manager.RedisClient") as MockRedisClient:
        ...
        # Mock pickle.dumps
        with patch(
            "app.utils.cache.redis_client.pickle.dumps",
            return_value=b"serialized_value",
        ):
            cache_manager = CacheManager()
